 ...
 * This class is Knight Class. Used to create Knight objects. This class has methods to 
 * set image for object. And uses Convenience methods to register mouse event handlers.
 * The Knight logic works entirely on mouse release and switch turns.  
...
public class Knight extends Piece {
	...
	//Constructor
	public Knight(String pieceName, String pieceColor, int x, int y, Image img) {
		super(pieceName, pieceColor);
		this.image = img;
		
		move(x , y);
		
		Rectangle knight = new Rectangle(Board.TILE_SIZE * 0.80, Board.TILE_SIZE * 0.80);
		knight.setFill(new ImagePattern(image));
		knight.setTranslateX(10);
		...
		setOnMouseReleased(e -> {
			if(TakeTurns.turn.contentEquals("Player1") && this.getPiece_color().contentEquals("Color1") || TakeTurns.turn.contentEquals("Player2") && this.getPiece_color().contentEquals("Color2")) {
				...
				//The logic works similar for both colored pieces.
					...
					//Check The Specific Spots
					if(newY == y0 - 2 && newX == x0 - 1 || newY == y0 - 2 && newX == x0 + 1 || newY == y0 - 1 && newX == x0 - 2 || newY == y0 - 1 && newX == x0 + 2 ||
					   newY == y0 + 2 && newX == x0 - 1 || newY == y0 + 2 && newX == x0 + 1 || newY == y0 + 1 && newX == x0 - 2 || newY == y0 + 1 && newX == x0 + 2 ) {
						//Kill Enemy Piece
